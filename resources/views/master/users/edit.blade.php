...
@section('content')
    <div class="row mb-4">
        <div class="col-12">
            <h3 class="fw-bold fs-3 mb-1">Edit User</h3>
            <p class="text-muted">Perbarui kredensial dan hak akses user: {{ $user->username }}</p>
        </div>
    </div>
                        ...
                        <div class="row g-4">
                            <div class="col-md-6">
                                ...
                                <select name="employee_id" id="employee_id"
                                    ...
                                    <option value="{{ $user->employee_id }}" selected>
                                        {{ $user->employee->name ?? 'Pilih Pegawai...' }}</option>
                                </select>
                                @error('employee_id')
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" name="username" id="username"
                                    ...
                                    placeholder="minimal 6 karakter, kecil semua, tanpa spasi" autocomplete="off">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="email" name="email" id="email"
                                    ...
                                    value="{{ old('email', $user->email) }}" placeholder="contoh@jmc.co.id">
                                @error('email')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-12">
                                ...
                                <p class="small text-muted mb-0"><i class="fas fa-info-circle me-1"></i> Biarkan password
                                    kosong jika tidak ingin mengubahnya.</p>
                            </div>
                            ...
                            <div class="col-md-6">
                                <label class="form-label fw-bold small text-muted">KETIK ULANG PASSWORD BARU</label>
                                <div class="input-group">
                            ...
                            <div class="col-12">
                                ...
                                <p class="form-text small mb-0">Jika dinonaktifkan, user akan langsung ter-logout otomatis.
                                </p>
                            </div>
                        </div>
                    </div>
        ...
        <script>
            $(document).ready(function () {
                ...
                $('#username').on('keyup input', function () {
                    ...
                    if (val.length < 6) { isValid = false; msg = 'Minimal 6 karakter.'; }
                    else if (/\s/.test(val)) { isValid = false; msg = 'Tidak boleh ada spasi.'; }
                    else if (!/^[a-z0-9]+$/.test(val)) { isValid = false; msg = 'Hanya boleh huruf kecil dan angka.'; }

                    if (!isValid) {
                        $(this).addClass('is-invalid-custom').removeClass('is-valid-custom');
                        feedback.text(msg).show();
                    } else {
                        $.get('{{ route("users.check-username") }}', { username: val, exclude_id: '{{ $user->id }}' }, function (res) {
                            if (res.exists) {
                ...
                $('#password').on('keyup input', function () {
                    if ($(this).val().length > 0) {
                        ...
                        $('#password').removeClass('is-valid-custom is-invalid-custom');
